...
ADMIN_IDS = {6595788533}  # Твой ID - не добавлять в таблицу
...
async def update_table_message(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if message_id:
            ...
            save_data()
            print(f"[Таблица] Создана в чате {chat_id}")
    except Exception as e:
...
async def sync_users_from_chat(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    """Синхронизирует участников чата с таблицей (исключая админа и ботов)"""
    init_chat(chat_id)
    
    try:
        # Получаем всех участников чата (админов и обычных)
        admins = await context.bot.get_chat_administrators(chat_id)
        
        # Пытаемся получить всех участников (если чат позволяет)
        all_members = set()
        ...
        # В супергруппе нельзя получить всех участников, только админов
        # Поэтому будем отслеживать только тех, кто активен
        ...
        # Удаляем тех, кого нет в чате
        for name in list(users.keys()):
            if name not in all_members:
                # Проверяем, не админ ли это
                is_admin_user = False
                for admin in admins:
                    admin_name = admin.user.username or admin.user.first_name
                    if admin_name == name and admin.user.id in ADMIN_IDS:
                        is_admin_user = True
                        break
                ...
                if not is_admin_user:
                    del users[name]
                    changed = True
                    print(f"[Синхронизация] Чат {chat_id}: удалён {name}")
...
async def handle_chat_member_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Игнорируем ботов (кроме бота1) и админа
    if user.is_bot:
        if user.id != BOT1_ID:
            return
    elif user.id in ADMIN_IDS:
        return  # Не добавляем админа в таблицу
...
async def handle_my_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Когда бота добавляют в чат - сразу создаёт таблицу"""
    ...
    if old_status in [ChatMemberStatus.LEFT, ChatMemberStatus.KICKED] and \
       new_status in [ChatMemberStatus.MEMBER, ChatMemberStatus.ADMINISTRATOR]:
        ...
        # Ждём, пока чат полностью загрузится
        await asyncio.sleep(3)
        
        # Создаём фейковый контекст
        fake_context = type('obj', (object,), {'bot': context.bot})()
        
        # Синхронизируем участников и создаём таблицу
        await sync_users_from_chat(chat_id, fake_context)
        
        # Убеждаемся, что таблица создалась
        await asyncio.sleep(1)
        await update_table_message(chat_id, fake_context)
        
        print(f"[Таблица создана] Чат {chat_id}")
...
async def cmd_upd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update_table_message(chat_id, context)
    print(f"[Команда /upd] Чат {chat_id} обновлён")
...
async def post_init(application: Application):
    ...
    print("🔄 Синхронизация существующих чатов...")
...
def main():
    ...
    print(f"   Админ (не добавляется): {ADMIN_IDS}")
    print(f"   Отслеживается бот1: {BOT1_ID}")
    print(f"   При добавлении в чат создаётся таблица")
