...
async def sync_users_from_chat(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    """Синхронизирует участников чата с таблицей (исключая админа)"""
    init_chat(chat_id)
    ...
    try:
        ...
        # Удаляем тех, кого нет в чате (кроме админа)
...
async def handle_my_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Когда бота добавляют в чат - создаёт таблицу"""
    ...
    if new_status == ChatMemberStatus.MEMBER or new_status == ChatMemberStatus.ADMINISTRATOR:
        ...
        # Небольшая задержка, чтобы чат успел синхронизироваться
...
def main():
    ...
    print(f"   Админ (не добавляется в таблицу): {ADMIN_IDS}")
